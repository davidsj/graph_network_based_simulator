...
class Trajectory:
    ...
    def __init__(self, positions, materials, n_materials, box_boundaries=None):
        ...
        if box_boundaries is not None:
            assert box_boundaries.shape[0] == self.dim
            assert box_boundaries.shape[1] == 2
    ...
    def save(self, path):
        np.savez_compressed(path, positions=self.positions, materials=self.materials,
                            n_materials=self.n_materials, box_boundaries=self.box_boundaries)

    @classmethod
    def load(cls, path):
        traj = np.load(path)
        return cls(traj['positions'], traj['materials'],
                   traj['n_materials'].item(), box_boundaries=traj['box_boundaries'])
